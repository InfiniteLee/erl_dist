...
use eetf::{Term, Atom, Tuple, Binary, Pid};
...
fn main() {
    ...
    let monitor = executor.spawn_monitor(TcpStream::connect(epmd_addr.clone())
        ...
        let from_pid = Pid::new(self_node0, 0, 0, 0);
        let atom = Atom::from("cnode");
        let binary = Binary::from(String::from(message).into_bytes());
        let mut msg: Vec<Term> = Vec::new();
        msg.push(Term::from(atom));
        msg.push(Term::from(binary));
        let tuple = Tuple::from(msg);
        let message = Message::reg_send(from_pid, dest_proc, tuple);
